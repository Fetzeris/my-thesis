...
tab1_names <- c( "Operating cash flow/ Average assets",  
                 ...
                 "PPENT ($ millions)", "PPENT/ Assets", 
                 "Financial covenant violation",
                 "New financial covenant violation", "One-time violator", "Frequent violator")
...
m3_atq <- feols(atq_change ~ viol_new  + ocf_atqaver + leverage + 
                  interestex_atqaver+ networth_atq + currentratio  + mb +
                  atq_log + ppentq_atq + atq_diff + ppentq_atq_diff | 
                  quarter + ffi + cqtr, # (4-1) cqtr dummies, ffi: 32 und 35 does not appeare in dataset: (38-2-1) = 35 dummies; quarter 19971 - 20081 -> 45-1 = 44  
                cluster = ~ gvkey + calender, # same results with: (1) ffi+ quarter and only (2) ffi and only (3) gvkey
                data = pdata) 
m4_atq <- feols(atq_change ~ viol_new  + viol_once + viol_multi + 
                    ...
                    quarter + ffi + cqtr, cluster = ~ gvkey + calender, data = pdata) 
...
table_reg1 <- texreg(list(m1_atq, m2_atq, m3_atq, m4_atq, 
                     ...
                     custom.gof.rows = list(`FE` = c("No", "No", "Yes", "Yes", "No", "No", "Yes", "Yes"),
                                            `Clustered SE` = c("No", "No", "Yes", "Yes", "No", "No", "Yes", "Yes")),
                     custom.coef.names = c("Constant", "New financial covenant violation",
                                           ...
                                           "PPENT/ Assets", "First differences Ln(Assets)", 
...
### save table ------------------------------------------------------------------
write_file(table_reg1, 'output/tables/my_Table3A_Regression.tex')
...
### save table ------------------------------------------------------------------
write_file(table_reg2, 'output/tables/my_Table3B_Regression.tex')
...
table_meandiff <- data.frame(matrix(ncol = 6, nrow = 15))
...
vars <- c("ocf_atqaver", "leverage", "interestex_atqaver", "networth_atq",
                  ...
                  "xrd_atq","atq","ppentq","ppentq_atq", "Number of observations", "Number of firms")
...
table_meandiff$OneTimeViolator[14] <- data %>% filter(viol_once ==1) %>% count()
table_meandiff$OneTimeViolator[15] <- data %>% filter(viol_once ==1) %>% group_by(gvkey) %>% select(gvkey) %>% distinct() %>% nrow()
table_meandiff$FrequentViolator[14] <- data %>% filter(viol_multi ==1) %>% count()
table_meandiff$FrequentViolator[15] <- data %>% filter(viol_multi ==1) %>% group_by(gvkey) %>% select(gvkey) %>% distinct() %>% nrow()
table_meandiff$X <- c("Operating cash flow/ Average assets",  
                 "Leverage", "Interest expense/ Average assets", 
                 "Net worth/ Assets", "Current ratio","Market-to-book", 
                 "Change in Ln(Assets)", "Change in Ln(PPENT)",    
                 "CAPEX/ Average assets", "R&D/ Assets", 
                 "Assets ($ millions)",  "PPENT ($ millions)", "PPENT/ Assets", 
